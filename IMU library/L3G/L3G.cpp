...
#define D20_SA0_HIGH_ADDRESS      0b1101011 
#define D20_SA0_LOW_ADDRESS       0b1101010 
...
bool L3G::init(deviceType device, sa0State sa0)
{
  ...
  {
    // check for L3GD20H if device is unidentified or was specified to be one of these types
    if (device == device_auto || device == device_D20H )
    {
      ...
      {
        // device responds to address 1101011; it's a D20H with SA0 high     
        sa0 = sa0_high;
      ...
      {
        // device responds to address 1101010; it's a D20H with SA0 low      
        sa0 = sa0_low;
        if (device == device_auto)
        {
          // use ID from WHO_AM_I register to determine device type
...
void L3G::enableDefault(void)
{
  ...
  // 0x6F = 0b11101111
  // DR = 01 (800 Hz ODR); BW = 10 (50 Hz bandwidth); PD = 1 (normal mode); 
