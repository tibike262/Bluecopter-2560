...
void FlightControl(){
  ...
  throttle=map(rxVal[0],THROTTLE_RMIN,THROTTLE_RMAX,THROTTLE_WMIN,THROTTLE_WMAX);

#ifdef RX_EXPO  //Software RX EXPO
  if(rxVal[3]<1480){
    setZ=map(rxVal[3],YAW_RMIN,1480,YAW_WMAX,0);
  }//if rxVal[3]
  else{
    setZ=map(rxVal[3],1520,YAW_RMAX,0,YAW_WMIN);
  }//else
  ...
  if((rxVal[3]>1480) & (rxVal[3]<1520)) setZ=0.0;

#else
  setZ=map(rxVal[3],YAW_RMIN,YAW_RMAX,YAW_WMAX,YAW_WMIN);
  if (rateAngleSwitch == 0){
